...
export default function AnimalsPage() {
  ...
  async function bulkDelete() {
    ...
    if (!confirm(`Permanently delete ${selected.size} animal(s)? This cannot be undone.`)) {
      return
    }
    setBusy(true)
    const supabase = createClient()
  ...
  async function toggleFlag(id: string, current: boolean) {
    ...
    setAnimals((prev) => prev.map((a) => (a.id === id ? { ...a, is_flagged: !current } : a)))
  }
