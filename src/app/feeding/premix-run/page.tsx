...
export default function PremixRunPage() {
  ...
  const kgLines = useMemo(() => {
    ...
    return lines.map((l) => ({
      ...l,
      kg: Number(((total * l.percent) / 100).toFixed(2)),
    }))
  }, [lines, amount])
    ...
    <div className="min-h-screen bg-slate-200">
      <header className="border-b-4 border-slate-600 bg-white px-4 py-4">
        <div className="mx-auto flex max-w-xl items-center justify-between gap-3">
          ...
          <Link href="/feeding" className="rounded-xl border-2 border-slate-500 bg-white px-4 py-2 text-sm font-bold">
            Back
          </Link>
        </div>
      </header>
          ...
          <>
            ...
            {premixes.length === 0 && (
              <p className="rounded-xl border-2 bg-white p-4 text-sm font-semibold text-slate-600">
                No premixes yet.{' '}
                <Link href="/feeding/premixes" className="underline">
                  Create one
                </Link>
              </p>
            )}
              ...
              {premixes.map((p) => (
                <li key={p.dietId}>
                  <button
                    ...
                    className="min-h-[72px] w-full rounded-2xl border-4 border-slate-600 bg-white p-4 text-left"
                  >
        ...
        {step === 'amount' && chosen && (
          <>
            ...
            <input
              ...
              className="min-h-[64px] w-full rounded-2xl border-4 border-slate-500 px-4 text-3xl font-bold"
            />
              ...
              {[250, 500, 750, 1000, 1500].map((n) => (
                <button
                  ...
                  className={`rounded-xl border-2 px-4 py-2 font-bold ${
                    Number(amount) === n ? 'border-brand-800 bg-brand-700 text-white' : 'border-slate-400 bg-white'
                  }`}
                >
                  {n} kg
                </button>
            ...
            <p className="text-sm font-semibold text-slate-600">This amount is remembered as the preset for next time.</p>
            <ul className="divide-y rounded-2xl border-2 bg-white">
              {kgLines.map((l) => (
                <li key={l.ingredientId} className="flex justify-between px-4 py-3 font-semibold">
                  <span>{l.name}</span>
                  <span>
                    {l.kg.toFixed(1)} kg <span className="text-slate-500">({l.percent}%)</span>
                  </span>
                </li>
              ))}
            </ul>
            <button type="button" onClick={goFill} className="min-h-[56px] w-full rounded-2xl bg-brand-700 text-lg font-bold text-white">
              Start loading
            </button>
          </>
        )}
        ...
        {step === 'fill' && current && (
          <>
            <p className="text-sm font-bold uppercase text-slate-600">
              Load {fillIndex + 1} of {kgLines.length}
            </p>
            <div className="rounded-3xl border-4 border-brand-900 bg-brand-700 p-6 text-white">
              ...
              <p className="mt-2 font-semibold text-brand-50">{current.percent}% of {amount} kg</p>
            </div>
            <ol className="space-y-1 text-sm font-semibold text-slate-700">
              {kgLines.map((l, i) => (
                <li key={l.ingredientId} className={i === fillIndex ? 'font-bold text-brand-800' : i < fillIndex ? 'text-slate-400 line-through' : ''}>
                  {i + 1}. {l.name} — {l.kg.toFixed(1)} kg
                </li>
              ))}
            </ol>
            <button
              type="button"
              onClick={() => {
                if (fillIndex + 1 >= kgLines.length) setStep('bay')
                else setFillIndex(fillIndex + 1)
              }}
              className="min-h-[56px] w-full rounded-2xl bg-brand-700 text-lg font-bold text-white"
            >
          ...
          <>
            <div className="rounded-3xl border-4 border-amber-700 bg-amber-50 p-6">
              ...
              <p className="mt-2 text-base font-semibold text-amber-900">
                {chosen?.name} · {amount} kg is mixed. Tip it into the premix bay, then mark completed.
              </p>
            </div>
            <ul className="divide-y rounded-2xl border-2 bg-white text-sm font-semibold">
              {kgLines.map((l) => (
                <li key={l.ingredientId} className="flex justify-between px-4 py-2">
                  <span>{l.name}</span>
                  <span>−{l.kg.toFixed(1)} kg stock</span>
                </li>
              ))}
              <li className="flex justify-between px-4 py-2 text-brand-800">
                <span>{chosen?.name} on hand</span>
                <span>+{Number(amount).toFixed(1)} kg</span>
              </li>
            </ul>
            <button
              type="button"
              disabled={saving}
              onClick={finish}
              className="min-h-[64px] w-full rounded-2xl bg-brand-800 text-xl font-bold text-white disabled:opacity-50"
            >
        ...
        {step === 'done' && (
          <div className="space-y-4 rounded-3xl border-4 border-green-800 bg-white p-6">
            ...
            <p className="font-semibold text-slate-700">
              {amount} kg of {chosen?.name} added to stock. Ingredients taken off main stock.
            </p>
            <button type="button" onClick={reset} className="min-h-[52px] w-full rounded-2xl bg-brand-700 font-bold text-white">
              Mix another batch
            </button>
            <Link href="/feeding/stock" className="block text-center font-bold text-brand-800 underline">
              Check stock
            </Link>
          </div>
        )}
